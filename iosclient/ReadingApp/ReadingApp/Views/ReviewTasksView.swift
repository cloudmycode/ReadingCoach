            ...
            if isExpanded {
                ForEach(group.tasks) { task in
                    WordReviewCompletedCard(
                        ...
                        articleTitle: task.articleTitle,
                        familiarityLabel: task.familiarityLabel,
                        isFamiliar: task.result?.lowercased() == "mastered",
                        onOpenArticle: {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                ...
                if let familiarityLabel {
                    Text(familiarityLabel)
                        .font(.system(size: 11, weight: .bold))
                        .foregroundColor(isFamiliar ? Color(red: 0.02, green: 0.7, blue: 0.44) : Color(red: 0.86, green: 0.35, blue: 0.28))
                        .padding(.horizontal, 8)
                        .padding(.vertical, 4)
                        .background(
                            (isFamiliar ? Color(red: 0.9, green: 0.98, blue: 0.94) : Color(red: 1.0, green: 0.94, blue: 0.93))
                        )
                        .clipShape(Capsule())
                }
                Spacer()
