        ...
        <div v-else class="day-group-list">
          <section v-for="group in completedDayGroups" :key="group.key" class="day-group">
            ...
            >
              ...
              <span class="day-group__chevron" :class="{ 'day-group__chevron--open': expandedDays.has(group.key) }">
                ›
              </span>
            </button>
              ...
              >
                <div class="record-card__main">
                  <div class="record-card__title-row">
                    <h3>{{ item.word }}</h3>
                    <span
                      v-if="familiarityLabel(item.result)"
                      class="record-badge"
                      :class="familiarityClass(item.result)"
                    >
                      {{ familiarityLabel(item.result) }}
                    </span>
                  </div>
                  <p class="record-card__meta">
                    {{ item.part_of_speech || "词性未标注" }} ·
...
function familiarityLabel(result?: string): string {
...
function familiarityClass(result?: string): string {
  switch ((result || "").toLowerCase()) {
    case "mastered":
