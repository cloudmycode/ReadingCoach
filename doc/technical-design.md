...
4. 当前默认接入 DeepSeek 作为 LLM，微软语音方案作为 TTS，同时保留可替换能力。
...
1. 缓存最近学习文章结构化数据
2. 缓存句子音频文件、音频 URL 与播放位置
3. 缓存当天统计摘要
4. 缓存登录态、最近同步时间和待同步任务

音频缓存策略：

1. 服务端为每个句子生成独立音频并保存到对象存储。
2. 客户端首次访问句子音频时从服务器下载并写入本地缓存。
3. 后续播放优先读取本地缓存，只有缓存缺失或失效时才重新下载。
4. 客户端可在“清理缓存”后重新按需下载音频。
...
1. 暂时使用 DeepSeek 作为文章清洗、句子拆分、翻译和拆句讲解的主模型。
2. 服务端封装统一的 `LLMProvider` 接口，避免业务层直接依赖单一模型厂商。
3. 关键输出采用 JSON Schema 或严格结构化约束，降低模型输出不稳定问题。
4. 对 OCR 清洗、句子拆分和翻译讲解分阶段调用 DeepSeek，避免单次 Prompt 过长。
...
1. 暂时使用微软语音方案，优先采用免费可用的语音模块完成 MVP。
2. 服务端封装统一的 `TTSProvider` 接口，后续如免费方案受限，可切换到 Azure Speech 正式服务或其他供应商。
3. 句子级音频优先生成并持久化保存到对象存储，减少重复调用。
...
1. MVP 阶段优先输出美式英语女声或男声中的一种，减少音色管理复杂度。
2. 音频文件统一转存到对象存储，不依赖第三方临时链接直接播放。
3. 对语音生成失败场景增加重试和降级机制，避免影响文章学习主流程。
4. 返回客户端的音频地址应为稳定可下载地址，便于客户端做本地缓存。
...
2. 对 DeepSeek 和微软语音接口调用增加 timeout、retry 和熔断保护。
3. 对登录、上传、同步等高风险接口增加限流和审计日志。

## 11. 成本控制建议

1. OCR 与 LLM 分步执行，避免不必要的音频生成
2. DeepSeek 调用按阶段拆分，减少单次长上下文成本
3. 音频按句缓存，重复打开不重复生成
4. 微软免费语音方案优先用于 MVP，控制早期成本
5. 免费用户限制每日解析篇数
6. 过长文章可按段落拆分处理
